...
(defun alfabeta (estado profundidade-limite peca-a-jogar f-utilidade alfa beta tempo-inicial tempo-maximo)
  (let* ((operadores (get-estado estado))
       (sucessores (sucessores-alfabeta (criar-no estado 0 (funcall f-utilidade estado (caixas-jogador-1 estado) (caixas-jogador-2 estado))) criar-operadores profundidade-limite peca-a-jogar f-utilidade (caixas-fechadas-j1 estado) (caixas-fechadas-j2 estado)))
       (sucessores-ordenados (sort sucessores (lambda (no1 no2) (> (utilidade no1) (utilidade no2))))))
    (if (>= (utilidade (first sucessores-ordenados)) beta)
      ...
      (let* ((maximo (max-side sucessores-ordenados profundidade-limite peca-a-jogar f-utilidade alfa beta tempo-inicial tempo-maximo))
           (novo-alfa (max alfa (utilidade (first maximo)))))
      (if (>= novo-alfa beta)
          (first maximo)
